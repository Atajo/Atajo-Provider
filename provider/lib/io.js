...
_io = {
  ...
  //   setInterval(function() { _io.processRequestCache(); }, 1000);
    ...
    _.SOCKET.on('connect', function() { _io.processConnect(); });

    _.SOCKET.removeAllListeners('disconnect');
    _.SOCKET.on('disconnect', function(reason) { _io.processDisconnect(reason); });


    //ECHO BASE HASH
    _.SOCKET.removeAllListeners('hash');
    _.SOCKET.on('hash', function() {   _.SOCKET.emit('hash', HASH); });


    //CORE EVENTS (STUB);
    _.SOCKET.removeAllListeners('event');
    _.SOCKET.on('event', function(data) {  _log.i("CORE EVENT : " + JSON.stringify(data));  });
    ...
    _.SOCKET.refreshBase = function() { var _ = _io;
      ...
      _.HASH = _base.hash(_.VERSION);

      //_log.d("BASE HASH FOR "+_.VERSION.BASEVERSION+" IS "+_.HASH);


      _base.build(_.VERSION, function(passed) {
        ...
        setTimeout(function() {
          ...
          _log.i("                            RELEASE HASH FOR VERSION "+_.VERSION.BASEVERSION+" --[ " + _.HASH + " ]--> UPLOADING TO CORE");
    ...
    _.SOCKET.on('get', function(obj) { var _ = _io;

      //TODO : clear object...

      //log the request
      _log.d("API GET ("+obj.handler+") : " + JSON.stringify(obj));
    ...
    _.SOCKET.on('ATAJO.SYNC.REQUEST', function(obj) { var _ = _io;

      //log the request
      _log.d("SYNC REQUEST : " + JSON.stringify(obj));
    ...
    _.SOCKET.removeAllListeners('req');
    _.SOCKET.on('ATAJO.SYNC.CHUNK', function(obj) { var _ = _io;

      //log the request
      _log.d("SYNC CHUNK REQUEST : " + JSON.stringify(obj));

      obj               = _.processLatency(obj);
      obj.responseEvent = "ATAJO.SYNC.CHUNK.RESPONSE";

      if(typeof obj.compressResponse == 'undefined') { obj.compressResponse = true; }
    ...
    // API CALL
    _.SOCKET.removeAllListeners('req');
    _.SOCKET.on('req', function(obj) { var _ = _io;
  ...
  initHandler : function(handlerName, obj) { var _ = this;
    ...
    if(!obj)
    {
       _log.e("INVALID REQUEST. DROPPING => "+JSON.stringify(obj));
       return;
    }

    var fnam = './handlers/'+handlerName+'.js';
    _log.d("CHECKING HANDLER "+fnam);
...
/*
           _log.d("REGISTERING PROCESS FOR "+fnam);
           ...
           {
              _log.e("DUPLICATE REQUEST FOR PID "+obj.pid+" => DROPPING");
              return;
           }
*/

           _log.d("FORKING PROCESS FOR "+fnam);
           //spawn the process
           ...
           HANDLER.on('message', function(msg)
           {
             ...
             {
                _log.e("COULD NOT PROCESS HANDLER RESPONSE");
             }
             ...
             {
               _log.d("HANDLER RESPONSE TYPE NOT RECOGNIZED");
             }
           ...
           HANDLER.send({request:obj, release:GLOBAL.RELEASE});
    ...
    processResponse : function(pid) { var _ = _io;
        ...
        fs.readFile(fnam, 'utf8', function(err, data) {
            ...
            {
              _log.e("COULD NOT PARSE RESPONSE (DROPPING) -> "+e);
            }
           ...
           if(obj)
            {
              ...
              if(obj.CHUNKED)
              {
                ...
                _log.d("CHUNKING "+arrLen+" ELEMENTS @ ~ "+ratio+" bytes PER ELEMENT");
                ...
                var chunks = [];
                  ...
                  for (var i=0, j=arrLen; i<j; i+=chunkSize) {
                    ...
                    _log.d("WE HAVE "+chunks.length+" CHUNKS");
                ...
                for(var i in chunks)
                {
                    ...
                    fs.writeFile(fnam, JSON.stringify(chunks[i]), function(err) {
                      if (err) {
                        _log.e("COULD NOT WRITE CHUNK TO CACHE FOR "+fnam+" [ "+err+" ]");
                      }
              ...
              else
              {
                ...
                _log.d('-->'+fnam);
                 ...
                 fs.unlink(fnam, function(err) { if(err) { _log.e("COULD NOT UNLINK "+fnam+" => "+err); } });
  ...
  processConnect : function() { var _ = this;
    ...
    var watcher = require('./watcher').init(function() { _.SOCKET.refreshBase(); });
  ...
  processDisconnect : function(reason) { var _ = this;
  ...
  getAPI : function() {  var _ = this;

    //  _log.d("INIT CORE API");
      var api = Object.create(API).INIT({ SOCKET : _.SOCKET, PARENT : _});
      return api;
